...
Um **Postgres**. Tabela `job_events`, schema criado na subida da API (sem Alembic). `data/events.jsonl` (10_000 eventos, janela de 24 h, versionado) substitui o S3 no desenvolvimento e é carregado **somente quando a tabela está vazia**.

O GET lê `job_events`, agrega S/F/Laplace em processo e aplica o quase-empate. Não lê o arquivo JSONL nem o S3. Sem cache de score e sem Redis.
...
- **Produção (só texto):** subir N réplicas stateless da API contra um Postgres compartilhado. Essa é a história de escalabilidade de R4. Sem k6, HPA, Redis, MinIO ou worker neste MVP.

## Alternativas

- Só arquivo: mais simples, mas as réplicas não compartilhariam um conjunto consistente de eventos (R4 mais fraco).
- Redis ou MinIO + worker: dois stores e um processo a mais. O ranking de N pools cabe em uma query mais memória; prematuro para o MVP.
