...
Como verificar a API à mão: unitário default, filtros e carga a 200 RPS. Contrato HTTP: [api.md](api.md). Score: [ADR 3](adr/003-score.md).
...
Aliases `/get-pool` e `/getpools`: mesmo contrato. O sorteio em massa fica no k6 (§3.1).
...
**O que prova:** R4 (pico de GET) **e** o quase-empate: 200 RPS iguais **não** vão 100% para o argmax.

k6: [load.js](../load.js). Default do arquivo é 20 RPS — **passe** `RPS=200`.

### 3.1 Sem filtro

```bash
curl -sf http://localhost:5050/get-pools
k6 run -e RPS=200 -e DURATION=30s load.js
```

| Métrica k6 | Esperado |
|---|---|
| `http_reqs` | ~200/s × 30 s ≈ 6000 |
| checks `status is 200` | ~100% |
| checks `body has pool_id` | ~100% |
| `pool_selected` (counter) | **dois** ids: `…-1a` e `…-1b` (ordem de metade/metade) |

No log: `near_tie` com **dois** membros em toda linha; `argmax_pool_id` sempre `…-1a`; `pool_id` alterna. Latência sobe um pouco (GET relê 10k). Falha de R4: 5xx ou checks < 100%.

### 3.2 Com filtro

```bash
k6 run -e RPS=200 -e DURATION=30s -e QUERY='category=memory' load.js
```

| Verificar | Esperado |
|---|---|
| Checks 200 | ~100% |
| `pool_id` | memory (`r6…`), mistura `1a` e `1b` (os dois são memory) |
| Log `filters.category` | `"memory"` |

Prova de conjunto unitário no pico: `-e QUERY='az=us-east-1a'` → counter só `…-1a`.

### 3.3 Leitura do relatório k6

- `http_req_failed` ≈ 0%.
- `http_req_duration` p95: anotar (não há SLO no MVP).
- Se `status is 200` cair: API saturada ou Postgres lento — ainda é o GET agregando 10k por request.
