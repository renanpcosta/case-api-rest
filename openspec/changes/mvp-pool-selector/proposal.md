...
- **Score Laplace** sobre todos os eventos do seed (10_000 linhas numa janela de 24 h; sem recorte no GET, sem decaimento): `S` = SUCCESS; `F` = SPOT_INSTANCE_TERMINATION + TIMED_OUT; SPARK_EXECUTION_ERROR persistido e ignorado no score; `score = (S+1)/(S+F+2)`. A agregação S/F corre **em cada GET**.
...
- **Docs e qualidade**: README, 3 ADRs (FastAPI, Postgres, score), `docs/api.md`, `docs/cenarios-de-teste.md`, testes unitários, CI com ruff + pytest. CD só descrito no README. Repo no GitHub.
